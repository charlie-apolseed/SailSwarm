/**********************
This program is designed to facilitate the autonomous navigation of a model sailboat between 2 specified points. 
...
int windDir = 0;         // Wind direction, input manually based on readings from wind vane
...
/***UPDATE BASED ON DESIRED POINS ***/
int xCordPointA = 0;   //The x cordinate of point A (in meters)
int yCordPointA = 0;   //The y cordinate of point A (in meters)
int xCordPointB = 10;  //The x cordinate of point B (in meters)
int yCordPointB = -10;  //The y cordinate of point B (in meters)


int targetAngle;  // Target direction of travel with reference to the wind direction. This will be determined using the 2 points. 
int targetHeading;
char *curAction;
...
void setup() {
  ...
  determineTargetAngle(xCordPointA, yCordPointA, xCordPointB, yCordPointB);  
...
/* Determines the target angle with respect to the wind based on the 2 input points */
void determineTargetAngle(int xCordA, int yCordA, int xCordB, int yCordB) {
  int dx = xCordB - xCordA;
  int dy = yCordB - yCordA;
  ...
  // Calculate target angle relative to wind direction
  targetAngle = round(theta) - windDir;
